  ...
  return (
    <article
      className={`relative flex flex-col rounded-3xl p-6 md:p-7 card-base ${
        plan.featured
          ? 'border-accent/50 ring-1 ring-accent/30 md:scale-[1.03] z-10'
          : ''
      }`}
    >
      {plan.badge && (
        <span className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full bg-accent text-[#05080F] text-xs font-bold uppercase tracking-wide">
          {plan.badge}
        </span>
      )}

      <p className="text-sm font-semibold text-muted-light mb-1">Plan {plan.name}</p>

      <div className="mb-5">
        <p className="font-syne text-3xl font-bold text-[var(--text)]">
          {plan.setup}
          <span className="text-base font-normal text-muted ml-1">{plan.setupNote}</span>
        </p>
        <p className="font-syne text-2xl font-bold text-accent mt-1">
          + {plan.monthly}
