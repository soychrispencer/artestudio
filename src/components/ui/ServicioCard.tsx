...
export function ServicioCard({ servicio }: { servicio: Servicio }) {
  return (
    ...
    >
      ...
      <span className="font-syne text-5xl font-extrabold text-white/[0.04] absolute top-4 right-6 select-none">
        {servicio.num}
      </span>
      <span className="text-3xl mb-4 block">{servicio.icon}</span>
      <h3 className="font-syne text-xl font-bold text-[var(--text)] mb-2">{servicio.title}</h3>
